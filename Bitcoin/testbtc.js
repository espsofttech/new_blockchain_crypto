...
var explorers = require('bitcore-explorers-minco');
		...
		if (request.params) {
			if (!request.params.walletAddress) {
				...
				if (walletAddress.length < 34 || walletAddress.length > 34) {
					...
					console.log('3...', balanceObj)

					if (balanceObj.response == '') {
						const balance = (balanceObj.balance / 100000000);
						var xmlHttp = new XMLHttpRequest();
						xmlHttp.open("GET", 'https://api.bitcore.io/api/BTC/testnet/address/' + walletAddress, false); // false for synchronous request
						xmlHttp.send();
						var transactions = JSON.parse(xmlHttp.responseText);
