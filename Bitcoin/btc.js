...
var Insight = require('bitcore-explorers-minco').Insight;
	...
	try {
		if (request.body) {
			...
			if (ValidationCheck == true) {
				...
				var balanceObj =  {response:''}
				//await getwalletbalance(_fromAddress);
				if (balanceObj.response == '') {
					let gasfee;
					let balance = balanceObj.balance * 100000000;
					if (request.params.gasValue) {
						gasfee = gasValue;
					} else {
						gasfee = 3000;
					}
					...
					if (balance >= (value + gasfee)) {
						var unspentObj ={response:''}
						// await getwalletnspent(_fromAddress);
			...
			if (!request.params.walletAddress) {
				...
				if (walletAddress.length < 34 || walletAddress.length > 34) {
					...
					if (balanceObj.response == '') {
						...
						// var xmlHttp = new XMLHttpRequest();
						// xmlHttp.open("GET", 'https://insight.bitpay.com/api/txs/?address=' + walletAddress, false); // false for synchronous request
						// xmlHttp.send();
						// var transactions = JSON.parse(xmlHttp.responseText);
						var date = new Date();
						var timestamp = date.getTime();
